...
The official documentation recommended the developer to use QList as the 'default' container in [Qt4](http://doc.qt.io/archives/qt-4.8/qlist.html), whereas from [Qt5](http://doc.qt.io/qt-5/qlist.html) the official documentation encourages developers to use `QVector` instead:
...
`QContiguousCache` functions in a similar manner to `QCache` but restricts the elements in the container to be contiguous. This enables the class to be more memory efficient than the `QCache` and use fewer processor cycles.
...
* Qt itself is moving away from the Qt containers and have replaced most of its internal structures with the STL ones.
